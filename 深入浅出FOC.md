...
我们已经求出每个矢量的作用时长了，接下来就是对应出定时器的占空比了。我们发现每个周期都需要2个产生力的矢量和一个零适量，为了让我们整体波形变换平滑，且对占空比容易把握，我们引入7段式PWM。首先我们先看一下每个矢量所对应桥臂的导通情况，也可以理解为相关信号的高低电平情况。即PWM波的情况。
...
其实到这里我想大家应该已经明白Clark变换和Park变换的作用力，我们拉开视角再看我们刚刚开环做了什么。
1.给出目标电流
...
经过变换之后我们就要面临电流采样问题了。通过上一章对7段式SVPWM的分析我们发现在一个周期内，不同时刻Mos的导通是不一样的，那么不同时刻对应的电流肯定也不一样，那么我们怎么拿到一个正确的电流呢？其实简单，因为电动机每项绕组可以分解为电感加电阻，而电感的作用是放置电流突变。由于一般采样电阻都是接在下桥臂，因此我们便可以在三个桥壁的下MOS管打开时刻进行电流采样。还是以第一扇区为例。
...
三电阻采样最为简单，在t6时刻采样即可，但是有人会为不是ia+ib+ic=0吗，那么我岂不是可以使用两个电阻采样而另一个用公式算出来，这当然可以，但是不要忘了，当我们的目标电流取最大时，a相上桥壁就会是全为高电平，这时候我们a相电流值就无法采样因此我们如果使用双电阻采样要将我们的电流最大值进一步限制，以留出来我们的采样时间，而三电阻就不存在这样的问题。

单电阻采样的电路要稍作变化。

![avatar](./单电阻采样电路.png)

其中在t0-t6不同时间采集到的电流含义不同

![avatar](./单电阻采样.png)
因此我们便可以在t4和t5时刻采样到-ic和ia电流完成电流采样
